...
Twenty-one signed-in screens live in [variant-b-impeccable/console/](variant-b-impeccable/console),
generated by `build-console.py` from `console_content.py` and `console_nav.py`. The builder imports
`build-pages.py`, so tokens, the wordmark symbol and the copy-rule checks are the same objects the
public pages use — they cannot drift. Review them all from `console/_screens.html`.

```bash
cd "open-portal-redesign/variant-b-impeccable"
python3 build-console.py
```

### Status colours — the one addition to the palette
...
`.app` two-column shell · `.side` sticky 252px sidebar, orange dash before each group label, `--card`
pill for the current screen · `.top` sticky topbar with the Ctrl-K search pill · `.kpi` statistic tile
· `.fbar` + `.sel` filter row · `.counts` status-count strip · `.st` / `.pill` status · `.empty` empty
...
plain guidance and the action that fills it.

### Two chart traps, both found by rendering

- **`.chart{width:100%}` eats a fixed-size dial.** The share ring is a 128px `<svg>` with width and
  height attributes; the class overrode both and drew it at panel width. Fixed-size charts need
  `.ring svg.chart{width:128px;height:128px;flex:none}`.
- **Two series on one axis hides the small one.** Requests (7,700) and failures (34) on a shared scale
  drew the failure line flat along the floor, which reads as zero failures. Split them: the volume chart
